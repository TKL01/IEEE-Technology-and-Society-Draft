...
\begin{document}
	...
	\title{Toward Extended Robot Intelligence: Task-Grounded Scene Graphs via Multimodal LLMs for Inclusive Human-Robot Teaming}
	...
	\begin{abstract}
A future robotized society will be characterized by effective and meaningful human-robot interactions, in which robots support human self-determination and well-being. However, it remains challenging to instruct a robot to retrieve objects or to follow a human teammate whose goals evolve dynamically. Moreover, robots rarely communicate their internal reasoning, making it difficult for users to calibrate their expectations. To address these limitations, we propose a hybrid, task-grounded, and human-centered intelligence framework for inclusive human-robot teaming. The system combines semantic object detection and segmentation with deterministic, rule- and constraint-based filtering to produce a structured representation of the scene, together with the kinematics and dynamics of the robot. We then process this enriched representation using Multimodal Large Language Models (MLLMs) for high-level semantic reasoning over verbal instructions, producing a task-grounded, human-interpretable, and actionable scene graph, which serves as a unified interface between perception, reasoning, and interaction. Unlike purely end-to-end or perception-only approaches, our framework explicitly integrates structured environmental and robot-state awareness with multimodal reasoning, enabling both interpretability and robust adaptability to novel tasks. The framework supports navigation, assistance, and guidance through natural language interaction, enabling users to collaborate with the robot intuitively regardless of technical expertise. Experimental results show that the proposed MLLM-enhanced scene graph improves perceived communication reliability, reasoning about previously unseen objects, and user guidance, while fostering a strong willingness to adopt the system for real-world applications.

	\end{abstract}
	
	\begin{IEEEkeywords}
		  Mobile robotics, Multimodal LLM, YOLO, Scene Graph, Human-robot teaming, Inclusion, Society of the future, pAIrSEEption, 3D scene understanding, Audio interface
	\end{IEEEkeywords}
...
\begin{itemize}
	\item {Hybrid task-grounded scene graph generation:} We combine deterministic preprocessing with rule- and constraint-based filtering and MLLM reasoning to generate semantically meaningful and physically feasible SGs that enable safe navigation and assistive interaction.

\item {From perception to semantic awareness:} We transform object-level perception into task-relevant, human-interpretable representations, promoting intuitive understanding and user self-determination.

\item {Human-centered and efficient design:} We introduce a voice-driven interface and generate SGs on demand from single frames, enabling intuitive interaction and deployment on resource-constrained devices.

\end{itemize}


The remainder of the paper is  as follows. Related work is presented in \cref{relatedwork}. The development of the SG is outlined in \cref{scenegraph}. The proposed methods and their implementation are described in \cref{methodology}. The experimental  results are presented in \cref{sec:experiments}. Finally, \cref{conclusion} concludes the paper.
...
Societal applications typically take place in unstructured environments, such as home settings, where scenes exhibit  spatial, temporal, and contextual dynamics. These changes arise from routine human activities, uncertain human intentions, appearance and disappearance of entities with  properties and affordances in the 3D scene \cite{yalcinkaya2024towards}. SGs provide a structured and multimodal representation of the sensed environment, allowing robots to reason about their operational space as well as determine and generalize appropriate actions. Neural-network-based approaches have therefore been proposed for object detection and scene graph generation (SGG) and interpretation in mobile robotics. The following subsections review relevant methods and outline their connections to our approach.

\subsection{Layered Architecture for Scene Graph}

Mobile robots can  reason over contextualized and structured information in order to plan and achieve goals. Recently, SGs have been adapted for mobile robotics, and multiple layers of abstraction of the real-world scene have been introduced for this purpose. These include the spatial layer (locations within the scene), the semantic layer (the meaning of entities, including their state, affordances, predicates, and other attributes), the topological layer (connectivity between locations), and the task-related layer (robot goals) \cite{rana2023sayplan,im2024egtr}. Entities  in the scene populate these layers, which differ in several respects. One such distinction is the update rate of entity attributes \cite{rana2023sayplan,im2024egtr}. For example, the robot initial and final positions may be known in advance and can therefore be treated as fixed, whereas task completion may be required within a short time window, prompting the robot to execute motions more quickly. In contrast, in entity-following applications, the robot desired position changes over time, indicating a dependence of the scene perception on the task and robot dynamics  often omitted.

	\begin{figure}[!t]
	\centering
	\includegraphics[width=\columnwidth]{pic/mental2.png}
	\caption{The robot semantically understands the \textit{outdoor} environment. It follows its human teammate \textit{from behind} while providing real-time, natural-language insights into the usefulness of surrounding objects, facilitating, e.g., smart mobility. These include obstacles (e.g., cars) and items relevant for active transportation (e.g., bicycles). By reasoning over relative pose, velocity, and object affordances, the robot assesses reachability and guides its teammate toward cycling. This supports a vision of future societies in which robots seamlessly assist daily  activities and foster the self-determination of humans.}
	\label{mental2}
\end{figure}

In this work, to ensure task relevance and physical feasibility, we employ a hybrid rule- and constraint-based filtering layer coupled with MLLM reasoning . Rule-based components encode spatial, semantic, and task-specific knowledge (e.g., distance thresholds, motion classification, and target selection), while constraint-based components enforce feasibility conditions such as safety, kinematics, and perception confidence. The resulting structured and task-grounded representation is used to construct a prompt for the MLLM, which performs high-level semantic reasoning. This allows the model to infer task-relevant relationships, resolve ambiguities, and generate human-interpretable scene descriptions. By combining rule-based preprocessing, constraint enforcement, and MLLM reasoning, the system produces a dynamic scene graph that is both semantically meaningful and operationally valid for real-world robotic tasks. Unlike purely rule-based SGs, our hybrid (rules, constraints, and MLLM) SG matures through  conversations with teammates and task-related experiences captured by the MLLM. This can empower citizens by simplifying their access to robotized assistance for self-determination in the society.
  \begin{table}[!t]
	\centering
...
Our robot functionality, termed \textit{pAIrSEEption}, is a multimodal natural language-driven system for environmental perception and navigation. It provides a responsive asynchronous graphical user interface (GUI) implemented with PySide6 (see Fig.~\ref{framework_architecture}), which serves as the interaction layer between the user and the system. Fig.~\ref{overview_framework} illustrates the main interface features. 

The GUI is platform-independent and can be deployed on different devices, such as the Jetson AGX Orin (32\,GB) edge platform or a standard laptop with an integrated Nvidia GPU. A ZED~2i stereo camera can be connected to these devices for perception. The PID-controlled mobile Husky UGV robot~\cite{husky} serves as the mounting platform for the hardware components (see Fig.~\ref{husky_robot}). The GUI serves as the visualization and control center of the system, where all component data streams are integrated. The interface displays the following information:

\begin{figure}[!t]
	\centering
	\includegraphics[width=2.5in]{images/overview_eng.png}
	\caption{Overview of features and components of the pAIrSEEption functionality.}
...
\begin{itemize}
	...
	\item System performance statistics, including frames per second (FPS) and utilization of the CPU, GPU, VRAM, and RAM.
	...
	\item Navigation controls for selecting detected objects to follow or approach.
	\item Internal robot diagnostics (e.g., motor temperatures) queried via voice commands and displayed in the GUI.
\end{itemize}

All displayed data, along with user voice inputs, can be stored locally in common file formats through the GUI. This enables dataset collection for potential future fine-tuning or training of the YOLO and MLLM models.
...
The system data flow is organized into five layers (see Fig.~\ref{framework_architecture}):
...
The object data, defined as \textit{object\_description}, are included in the prompt and converted into concise bullet points during preprocessing. Initially, a template was used to structure the raw data. Fig.~\ref{transformation_template} illustrates the result of this deterministic transformation.
By combining image input with the structured object data, the MLLM generates a human-readable description of the 3D scene. In addition, a scene graph (SG) is constructed by enriching the detected object data with semantic descriptions produced by the MLLM. These outputs are stored as CSV and text files, forming a local structured memory for the system. This enables the model to request clarification in ambiguous situations, for example when multiple objects of the same class (e.g., cars) appear in the scene.
The generated scene interpretation is displayed in the GUI output window and can optionally be played back using the text-to-speech (TTS) system. Such functionality can assist blind or visually impaired users during navigation. An example of this semantic scene interpretation is presented in Section~\ref{subsec:sceneunderstanding}.
...
\subsubsection{Phase 1: On-site \textcolor{red}{(Expert?)} Evaluation, Setup and  Design}

For the on-site testing, participants ($N = 9$) with technical or engineering backgrounds (\textcolor{red}{approx. age range, genders?}) interacted with the Husky mobile robot indoors using German voice commands (e.g., ``Folge Person~1'' = ``Follow person~1''). After a brief introduction to the GUI and system functionality, participants instructed the robot to follow selected targets over distances chosen by themselves. Voice commands could also be used to stop navigation or switch to a different target. During operation, the robot provided audio feedback describing its actions. After completing the interaction, participants anonymously evaluated their experience through a survey.
...
\begin{itemize}[leftmargin=*]
	...
	\begin{enumerate}[label=Q\arabic*:, series=survey, widest=Q12, leftmargin=*]
		\item Ease of issuing commands. 
		\item Reliability of command recognition. 
	...
	\begin{enumerate}[label=Q\arabic*:, resume=survey, widest=Q12, leftmargin=*]
		\item Accuracy of object identification.
		\item System response time to navigation commands. 
	...
	\begin{enumerate}[label=Q\arabic*:, resume=survey, widest=Q12, leftmargin=*]
		...
		\item Suggestions for improving user experience or comfort.
...
High ratings were also observed for command recognition reliability, system response time, and overall usability ($\bar{x}_{Q2,Q5,Q7} = 4.56$), suggesting that the tracking behavior was perceived as predictable and safe. Participants also expressed a strong willingness to use such a system in real-world applications ($\bar{x}_{Q9} = 4.22$). Conversely, the low scores for the necessity of command repetition ($\bar{x}_{Q3} = 1.75$) and the frequency of losing the tracked target ($\bar{x}_{Q6} = 1.56$) further indicate that both the voice control and object tracking performed reliably. The generally low standard deviations across items suggest a strong consensus among participants.

However, several limitations should be noted. All participants had engineering backgrounds and were undergraduate- or graduate-level students, implying a considerable  technical familiarity and intrinsic interest in robotics. Consequently, perceived usability and acceptance may differ for non-expert users. In addition, the  sample size ($N=9$) limits the generalizability of the findings and therefore represents only a pilot evaluation.
...
Participants also provided qualitative feedback (Q10--Q12) highlighting both operational performance and potential system improvements. Positive remarks emphasized the precision and responsiveness of the system, the clarity of the interface, and the stable distance maintenance during following tasks. Suggested improvements included smoother turning behavior when navigating around corners and broader language support. While the current architecture uses \textit{faster-whisper}~\cite{fasterwhisper} for automatic language recognition of voice commands, the STT output is currently limited to German. Since the integrated MLLMs support multilingual interaction, extending language support remains an important direction for future development.

Additional recommendations included improving object re-identification (Re-ID), enhancing obstacle detection, and providing clearer guidance for voice commands. Participants also suggested new functions such as a reverse driving mode, a 180$^\circ$ turn command, the integration of a robotic arm, and inter-robot communication. Furthermore, fallback mechanisms for handling voice-signal loss and additional communication modalities were proposed to improve robustness. Overall, the results suggest that the proposed system can lower the barrier to human–robot interaction and shows strong potential for service robotics applications in domestic and industrial environments.

\subsubsection{Phase 2: Global  Perception Study}
The second group comprised 10 additional participants without prior robotics experience, representing a broad demographic spectrum (ages 8–74 years, mean $\bar{x}_{age} = 43.9$, five male and five female) across Africa, Asia, and Europe. In this second study, participants were asked to provide their opinions based on two videos: one showing verbal interactions between humans and the robot, and another demonstrating the robot navigation and object-tracking capabilities. The qualitative assessment was structured around three compressed questions, similar to Phase I: interaction quality, (navigational/system) trust, and practical utility. The qualitative results indicate a comparable level of perceived usefulness to that observed in Phase I. Nine participants praised the interaction as natural, reflecting generally positive perceptions, while one participant did not provide a response. Participants reported high trust in the navigation and conversational capabilities, with particularly strong confidence among younger Europeans. However, two participants based in Africa emphasize the need to strengthen technical reliability in dynamic or structurally challenging environments (e.g., demanding road infrastructures, flooded  areas). These insights point to the need to  enhance the SG-based semantic  understanding of seasonal contexts, reasoning, and navigation of the robotic system. Practical potential was identified in logistics, agriculture, retail. A notable surprise was the high acceptance of the inclusive robotized system among remote participants, particularly for elderly care and daily life assistance  in Africa. Access to relevant data and model may be the Feedback from Africa and Asia further highlights the importance of supporting local languages to foster a more global accessibility and experience. Overall, the second-phase study indicates that social acceptance was generally high and positively perceived, even among laypersons.


\section{Conclusion}\label{conclusion}

This work introduced a hybrid, task-grounded framework for scene graph generation that integrates precise 3D sensor data and the semantic reasoning capabilities of MLLMs for mobile robotics. By transforming structured metric object data into natural language representations, the proposed approach bridges perception and semantic awareness, enabling both human interpretability and actionable understanding of the environment. The resulting scene graphs support intuitive navigation, assistance, and guidance, making robotic behavior more transparent and accessible, particularly for users with limited visual perception. Experimental results demonstrate that grounding MLLM reasoning in sensor data leads to factually consistent scene interpretations, while the user studies confirms high perceived reliability, safety, and (social) acceptance. Overall, this work advances toward extended, human-centered robot intelligence, where robots can reason about their environment and communicate this understanding at high level to support inclusive and meaningful human-robot collaboration. Our framework empowers users to better understand, guide, and collaborate with robotic systems in real-world environments, paving the ground for future robotized society.
...
%	\bibitem{singh2024pushing}	Singh, C. and Jayadas, A. Pushing Shopping Cart: An Intentional Exercise for Older Adults. {\em Activities, Adaptation \& Aging}. \textbf{48}, 670-689 (2024)
	...
	\bibitem{kaigom2023metarobotics}Guiffo Kaigom, E. Metarobotics for industry and society: Vision, technologies, and opportunities. {\em IEEE Transactions On Industrial Informatics}. \textbf{20}, 5725-5736 (2023)


	
	\bibitem{konnecke2024}
	A.~E.~K{\"o}nnecke, R.~Harmann, and E.~Guiffo Kaigom,
	``Human-Follower Mobile Robot With High Payload for Cross-Domain Assistance Using Shared Autonomy,''
	in \textit{Proc. Conference of the International Society for Gerontechnology}, 2024, pp.~1--1.
	
	\bibitem{la2024toward}
	T.~K.~La and E.~Guiffo Kaigom,
	...
	in \textit{Proc. IEEE Conference on Telepresence}, 2024, pp.~179--182.


\bibitem{kruger2019programming}
J.~Kr{\"u}ger, T.~K.~Lau, and A.~Verl,
``Programming Flexible Robot Systems: Challenges and Opportunities,''
\textit{Procedia CIRP}, vol.~81,  2019.

\bibitem{ajoudani2018progress}
A.~Ajoudani, A.~Billard, P.~Kormushev, et al.,
``Progress and Prospects of the Human–Robot Collaboration,''
\textit{Autonomous Robots}, vol.~42, no.~5, pp.~957--975, 2018.

\bibitem{beetz2018knowrob}
M.~Beetz, D.~Be{\ss}ler, A.~Haidu, M.~Pomarlan, A.~Bozcuo{\u{g}}lu, and G.~Bartels,
``KnowRob 2.0 --- A Knowledge Processing Framework for Cognition-Enabled Robotic Agents,''
in \textit{Proc. IEEE Int. Conf. on Robotics and Automation (ICRA)}, 2018.

\bibitem{pedersen2016robotics}
M.~R.~Pedersen, S.~Fuglsang, K.~Kragh, et al.,
``Robot Skills for Manufacturing: From Concept to Industrial Deployment,''
\textit{Robotics and Computer-Integrated Manufacturing}, vol.~37, pp.~282--291, 2016.



\bibitem{chen2020vision}
Z.~Chen, Z.~Wang, and H.~Liu,
``Vision-Based Human Following for Mobile Robots: A Survey,''
\textit{IEEE Access}, vol.~8, 2020.

\bibitem{wu2021person}
C.~Wu et al.,
``Person-Following Robot: A Survey and Future Directions,''
\textit{IEEE Transactions on Human-Machine Systems}, 2021.

\bibitem{li2022robust}
J.~Li et al.,
``Robust Human-Following Control for Mobile Robots in Dynamic Environments,''
\textit{Robotics and Autonomous Systems}, 2022.

%\bibitem{shah2023lmnav}D.~Shah, B.~Osiński, S.~Levine, and others,``LM-Nav: Robotic Navigation with Large Pre-Trained Models of Language, Vision, and Action,''\textit{arXiv preprint arXiv:2303.07376}, 2023.

\bibitem{gupta2023vlmaps}
S.~Gupta, V.~Tolani, S.~Levine, and others,
``VLMaps: Building Object-Centric Visual Maps for Robot Navigation,''
in \textit{Proc. IEEE International Conference on Robotics and Automation (ICRA)}, 2023.

\bibitem{huang2023vlfm}
W.~Huang, C.~Wang, Y.~Liang, et al.,
``Visual Language Maps for Robot Navigation,''
\textit{arXiv preprint arXiv:2303.10894}, 2023.

\bibitem{konishi2023navgpt}
K.~Konishi, S.~Huang, and others,
``NavGPT: Explicit Reasoning in Vision-and-Language Navigation with Large Language Models,''
\textit{arXiv preprint arXiv:2305.16986}, 2023.
...
\bibitem{rana2023sayplan}Rana, K., Haviland, J., Garg, S., Abou-Chakra, J., Reid, I. \& Suenderhauf, N. Sayplan: Grounding large language models using 3d scene graphs for scalable robot task planning. {\em  Preprint ArXiv:2307.06135}. (2023)

\bibitem{im2024egtr}Im, J., Nam, J., Park, N., Lee, H. \& Park, S. Egtr: Extracting graph from transformer for scene graph generation. {\em Proceedings Of The IEEE/CVF Conference On Computer Vision And Pattern Recognition}. (2024)
