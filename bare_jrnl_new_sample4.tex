...
\subsection{Task-Grounding} \label{taskgrounding}
Efficient YOLO-based object detection serves three purposes in the system. First, YOLO provides real-time 2D detections that are fused with ZED~2i depth measurements via the ZED SDK to estimate spatial object information (e.g., distance and 3D position; see Fig.~\ref{framework_architecture}). These data are then formatted (Fig.~\ref{transformation_template}) and passed to the MLLM as part of the grounded input prompt. Second, although modern MLLMs/VLMs can generate 2D bounding boxes, YOLO is employed as a lightweight preliminary detection layer to reduce the computational load on the MLLM and minimize inference latency. Third, the detections enable navigation and object-following. For this purpose, the distance between each detected object and the camera, as well as pairwise inter-object distances within a frame, are computed using the 3D Euclidean metric. The target object is selected through its unique identifier (ID), which can be specified either via the graphical interface or by voice input.
To enable the latter, the system performs semantic task mapping by translating unstructured and potentially noisy human speech into structured, executable machine commands. Voice transcription is facilitated via faster-whisper \cite{fasterwhisper}. The transcription then goes through a two-stage process. First, an integrated speech intent parser uses rule-based pattern matching via regex rules (Fig.~\ref{regex})  [PLACEHOLDER GERMAN IMG HERE] . Second, a fuzzy string matching is used to correct common transcription errors (misinterpreted words) and map spoken requests to robot commands such as "navigate" or "follow", in case the regex rules fail to recognize the intent. At the same time, spoken object names are mapped to valid YOLO classes, ensuring that the human’s verbal intent is strictly grounded with the robot’s perceptual capabilities.

\begin{figure}[htpb]
	\centering
	\includegraphics[width=\columnwidth]{images/regex.png}
	\caption{Need translation and new design, resize.}
	\label{regex}
\end{figure}
...
These raw spatial coordinates are then transformed into actionable knowledge, the system maps geometric states directly to the user's verbalized task. Furthermore, before this grounded object data reaches the MLLM, rule-based filtering applies physical and kinematic constraints. For example, the operator can determine the distance to the object that should be maintained or stop the robot in critical situations using a voice command.
...
The object data, defined as \textit{object\_description}, are included in the prompt and converted into concise bullet points during preprocessing. Initially, a template was employed to structure the raw data. Fig.~\ref{transformation_template} illustrates the result of this deterministic transformation. By combining structured object data with image inputs, the MLLM generates a human-readable description of the 3D scene, as demonstrated in Fig.~\ref{transformation_template}. 

\subsection{Memory Augmentation} \label{memoryaugmentation}
Unlike traditional perception-only pipelines, the  SG is constructed by enriching and fusing  detected and structured object data along with robot-state information with MLLM-generated semantic and task-grounded properties, forming a memory-augmented SG. These outputs are stored as CSV and text files, creating a local structured memory that allows the system to maintain context over time and request clarification in ambiguous situations. An  example is when multiple objects of the same class (e.g., cars) appear in the scene (PLACEHOLDER Fig.~\ref{scenememory}). Integrating structured perception, memory, and reasoning enables context-aware, human-guided interaction, marking a significant advancement over conventional SG approaches.

\begin{figure}[htpb]
	\centering
	\includegraphics[width=\columnwidth]{images/scenememory.png}
	\caption{Placeholder, need translation and resize or new design.}
	\label{scenememory}
\end{figure}

The memory architecture in pAIrSEEption combines short-term and long-term memory. To ensure robustness against sensor noise and visual occlusions, the short-term memory stores the most recently known attributes of objects (e.g., identity, spatial coordinates, and task role) and reapplies them when the object re-enters the field of view (see Fig. \ref{renentrance}). If an object is temporarily lost due to low recognition confidence, the memory-based SG uses a timeout hysteresis to maintain the object’s state based on the most recent observations, thereby preventing target loss.
To establish a long-term context, the system processes historical tracking data (e.g., object logs and distance metrics). By analyzing these logs, the module determines the frequency of object observations, common interaction patterns, and average confidence values. This historical data serves as a context layer and informs the MLLM about the stability of certain environmental features. To comply with the constraints of the context window, only relevant historical summaries and calculated metrics are appended to the system prompt. Furthermore, in ambiguous situations involving multiple objects of the same class, the navigation agent relies on this historical and contextual data to formulate clarifying  voice-based questions (e.g., “I see car 1 and car 5 - which one do you mean?”). The operator can then decide how to interact next. This mechanism shifts the system from a passive scene description to active, memory-based disambiguation.
