...
\begin{document}
	...
	\begin{abstract}
A future robotized society will be characterized by effective and meaningful human-robot interactions, in which robots are seamlessly integrated into daily life to support self-determination and well-being. However, most current robotic systems are largely designed for scripted behaviors and require significant programming effort, which limits their broader adoption. It remains challenging to verbally instruct a robot to retrieve previously unseen objects or to follow a human teammate whose goals evolve dynamically. At the same time, robots rarely communicate their internal reasoning about the environment, making it difficult for users to understand system behavior and adjust their expectations accordingly. To address these limitations, we propose a hybrid, task-grounded, and human-centered intelligence framework for inclusive human-robot teaming. The system combines semantic object detection and segmentation (YOLO) with deterministic, rule- and constraint-based filtering to produce a structured scene representation. This representation is then processed by Multimodal Large Language Models (MLLMs) for high-level semantic reasoning, resulting in a task-grounded, human-interpretable, and actionable scene graph. The framework supports navigation, assistance, and guidance through natural language interaction, enabling users to collaborate with the robot intuitively regardless of technical expertise. Experimental results indicate that the proposed MLLM-enhanced scene graph approach improves perceived communication reliability, handling of previously unseen objects, and user safety, while fostering a strong willingness to adopt the system for real-world applications.
...
\subsubsection{Phase 2: Global \textcolor{red}{(Layperson?)} Perception Study \textcolor{red}{(Video-based?)}}
The second group ($N=10$) comprised participants/\textcolor{red}{laypersons} with no prior background in robotics, representing a broad demographic spectrum (ages 8–74 with mean $\bar{x}_{age} = 43.9$, five male and five female participants) across Africa, Asia and Europe. In this second study, participants were asked to give their opinion based on a video of the verbal interaction, as well as the navigation and object tracking of the robot. The qualitative assessment was structured around three key dimensions similar to phase 1, but compressed: 
Interaction quality, (navigational/system) trust and practical utility. The qualitative results reveal a similar high acceptance as in phase I. The majority participants described and praised the interaction as natural. The trust in navigation was also perceived high, however, skepticism remains regarding technical reliability in dynamic or structurally challenging environments, particularly when it comes to a wheeled mobile robot. Practical potential was identified in the sectors of logistics, agriculture, retail, and elderly care. The feedback further emphasizes the need for multi-language support to ensure accessibility. Overall, the second phase study shows that even among laypersons and non-expert users, social acceptance was generally high and viewed positively.
