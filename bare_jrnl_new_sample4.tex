...
\begin{document}
	...
	\begin{figure}[!t]
		...
		\caption{Perception of an \textit{indoor} environment by a mobile robot. Semantic detection and segmentation of objects to construct a scene graph as a mental model. The robot reasons on this graph using an MLLM to track a moving chair mentioned by the human-teammate using natural language.}
		\label{mental1}
	\end{figure}
...
The remainder of this work is organized as follows.
	...
	\caption{Semantic understanding of an \textit{outdoor} societal workspace by the robot that not only follows the human teammate but also gives hints about surrounding objects using  natural language. Objects are e.g. obstacles, such as a car, or  useful items like a bicycle. The robot is  aware of an estimated relative pose  (i.e., position and orientation) and velocity along with the affordance of objects even unseen by humans. It uses this information for  reachability assessment and human guidance. The robot therefore uses AI to visually and semantically understand the scene (hence, we coined it \textit{pAIrSEEption}) as well as inform and  augment humans, paving the ground for the society of the future.}
	\label{mental2}
\end{figure}
...
Robotized societal applications are typically characterized by unstructured environments, such as home settings, where scenes exhibit diverse spatial, temporal, and contextual dynamics. These dynamics are shaped by routine human activities as well as irregularities and uncertainties arising for instance from human intentions along with the appearance and disappearance of entities with disparate properties and affordances in the 3D scene \cite{yalcinkaya2024towards}. A scene graph (SG) provides a structured hierarchy and multi-modal representation  of the sensed environment about which the robot can reason to understand its operational space and decide how to act. The ability of statistical methods propelled by neural networks to handle uncertainty and generalize across scenarios has promoted their employment for object detection and scene graph generation (SGG) in mobile robotics. In the next sections, we provide an overview of methods developed to this end and outline connections to our approach.

%Scene graph generation (SGG)  requires the identification, localization, and interpretation of entities termed nodes along with their attributes and the relationships between them, which are represented as edges \cite{rana2023sayplan}. The ability of statistical methods based on neural networks to handle uncertainty and generalize across scenarios has promoted their use in object detection and SGG for mobile robotics. In the next sections, we provide an overview of methods developed to this end and outline our approach.
 


 
\subsection{Layered architecture for scene graph}
Mobile robots need to reason about contextualized and structured information to plan and achieve goals. SGs have recently been adapted for mobile robots and different layers of abstraction of the real scene have been introduced for this purpose. Included are the spatial (locations within the scene), semantic (meaning of entities, including their state, affordances, predicates, and further attributes), topological (connectivity between locations), and task-related (robot goals) layers \cite{rana2023sayplan,im2024egtr}. Entities present in the scene populated these layers, which differ in various ways. The update rate of attributes of these entities is an example of such a differentiation \cite{rana2023sayplan,im2024egtr}. For instance, while the initial and final positions of the robot may be known in advance and thus fixed, the achievement of the task itself  may be required within a short time window. To this end, the robot is commanded to complete motions faster. By contrast, the desired position of the robot varies with time in object-following applications. In this work, we leverage a large language model to enable an additional maturation layer rooted in the task and interaction history of the mobile robot. Hence, our SG  matures from experiences.
  \begin{table}[!t]
	\centering
...
Three classes of algorithms currently emerge for the design of SGs, as emphasized in \cref{tab:features}.
Edge dual scene graph generation has been recently proposed   to capture multi-object relationships for SGG \cite{kim2025semantic}. The approach integrates object detection based on a fast Region-based Convolutional Neural Networks (R-CNN), an edge dual scene graph,  and an object-relation-centric Message Passing Neural Network (MPNN). Rich contextual interactions  between unconstrained objects are retrieved by the object-relation MPNN that learns both object- and relation-centric features to enhance the prediction accuracy and enable  fine-grained and iterative updates between objects. In orther words, MPNN aggregates information from neighborhood entities. Updates of the description of the considered entity are thereby enabled. MPNN finally outputs a feature vector for the entire graph \cite{kim2025semantic,su2024adopting}. Further modern techniques for SGG leverage the attention mechanism. Such  approaches connect objects with significant attention weights to edges from the self-attention layers of a pre-trained DEtection TRansformer  \cite{im2024egtr}. In the third strategy based on Convolutional Scene Graph Generation (CSGG), objects are encoded as bounding box center points. Relationships are represented  as 2D vector fields, i.e., an integral on a sub-region that points from subject to object \cite{liu2021fully}. Features related to the MPNN-, attention-, and CSGG-based approach are given in \cite{liu2024repsgg} and associated in 	\cref{tab:features} with their inference methods. \textcolor{red}{Herr La: sind wir box-, query- and/oder point based?  We use a box-based approach (YOLO, CNN) and extend it by employing an MLLM to determine relations between objects through a combination of deterministic preprocessing (see Fig. \ref{transformation_template}), natural language and logical inference.}


\subsection{Tools for the genereration of scene graphs}

Different tools have been developed  to carry out a set of intermediate operations required for SGG from sensed data. An overview of these tools is provided in \cref{tab:scenegraph_technologies}. For instance, YOLO (You Only Look Once) and SAM (Segment Anything Model) play major roles in object detection and segmentation tasks. SAM is a general-purpose model capable of pixel-level segmentation, whereas YOLO primarily provides bounding-box detection. YOLO is purposefully optimized for low latency, making it suitable for real-time scenarios in dynamic, human-centered environments where rapid collision avoidance is critical. This enables faster collision handling while following human motion paths in our framework.
...
Scene graphs have been deployed for different societal applications \cite{amiri2022reasoning}. Finding someone a place to sit in an indoor environment \cite{ravichandran2025safety}, assistance in cooking \cite{kwon2025large}, and  inspection missions in city-scale outdoor environments \cite{viswanathan2024xflie} are application examples for the society of the future. Although most SGs capture information from an image, human activities are usually characterized by a long-term horizon that goes beyond a single image.

 Long-term goals in  human-inhabited environments are handled in \cite{ravichandran2025safety} by federating multiple local SGs from previous pictures to generate a global SG.  In this work, we leverage a MLLM that exploits the reasoning history and its generalization capability to achieve long-term reasoning. Such a contextual grounding is also enabled in \cite{kwon2025large} using the generative capability of an LLM in conjunction with preceding scenarios. We enlarge the scope of this scene-centered contextualization. Our goal is to integrate the mobile robotic device in the collective intelligence loop, as advocated in \cref{introi} and in Metarobotics framework \cite{kaigom2023metarobotics}. Specifically, internal processes of the robot, such as its energy consumption and motion dynamics, are taken into account. This allows for the management of both the  planning and physical feasibility of tasks. 
 
 Spatial goals specified by a human teammate and requirements on the robot dynamics are considered in the communication, taking care to keep the vocabulary as accessible as possible. A human teammate can thereby ask  the robot whether it has localized the target object in its current SG-based mental model and can physically reach that object long before an  attempt to move to the object is started. Visual question-answering queries have been used in \cite{morales2025vqa} to discover relevant events for a safe navigation through a city  and then build a SG that structures gained information to assist people with low vision in urban traffics. The device  that supports this assistive capability is a forward-facing glass \cite{morales2025vqa}. In this work, the mobile robot not only follows  but also informs and guides its teammate from behind with a $360^{\circ}$ perspective on the environment  while dodging  societal obstacles.
...
An SG serves to represent a scene with objects,
their attributes, and relationships between other objects in a structured way.
SGG can be based on images, text, and videos.
...
$\langle \text{subject} \;-\; \text{predicate} \;-\; \text{object} \rangle$ triple. \cite{hli2024sgg} 
...
Due to the steadily increasing availability and improvements of Foundation Models, there has been a significant shift from class limited object recognition to universal segmentation models (e.g.,  SAM/SAM 2) and open-vocabulary encoders (e.g., CLIP) that provide pixel-accurate masks and semantic embeddings, respectively. This allows modern MLLMs and Vision Language Models (VLM, multimodal models that can handle text and visual input) to recognize not only object classes but also their semantic states and complex relationships. Frameworks like ConceptGraphs and 3DGraphLLM use this information to construct scene graphs that are interpretable for LLMs and ground these objects in 3D space by integrating geometric metadata from sensors such as LiDAR or depth cameras. Approaches such as VLMaps and Graph2Nav focus on directly translating these spatial relations into executable navigation commands. 
Some of the frameworks presented require prior mapping or 3D reconstruction \cite{3dgraphllm}, \cite{hovsg}, \cite{openin}, \cite{interaction}, while others are designed for use in unknown environments and create a map in real time to navigate \cite{conceptgraphs}, \cite{graph2nav},\cite{sgnav},\cite{lgx}, \cite{vlmaps}. However, these require powerful hardware, various sensors and further processing by various systems. In most cases, an entire room or floor must first be scanned/mapped using LiDAR and/or depth cameras. This map is then further processed and enriched with descriptions of these objects, for example with the help of MLLMs. This enables, for example, Visual Question Answering (VAQ) about specific objects. In contrast, our approach is designed to be as resource-efficient and cost-effective as possible. It simply creates a scene graph of the current image (a snapshot) and does so only when required, e.g., when the current scene needs to be described. Therefore, it is dynamic and does not require time-consuming mapping beforehand.
...
%Where the object $o_1$ receives the attributes ID, confidence value, 3D positions, 3D velocity, and the classification \textit{static} if the velocity is zero. Thus, \textit{static} is an attribute derived from velocity, and the attribute set \textit{A} (see (\ref{eq:o_i})) is expanded with this additional information. 
...
%Fig. \ref{client_server} depicts the implemented data pipeline, where the Jetson acts as the client. The Jetson is used here as a representative PC on which the GUI application runs. However, we also show that even consumer laptops can be used as a client. Due to the computing capacity of the Jetson Orin (32 GB or 64 GB), it is also possible to use it as a server for smaller local MLLMs.
...
%\subsection{Lessons learned here?}
...
%\begin{figure}[!ht]
%	\centering
%	\includegraphics[width=\columnwidth] {images/yolo_performance_gpu_big.png}
...
%	}
%\end{center}
%*The response from the Gemma 3 12 B model was originally generated in German.
