...
\subsubsection{Phase I: On-site \textcolor{red}{(Expert?)} Evaluation, Setup and  Design}
...
Additional recommendations included improving object re-identification (Re-ID), enhancing obstacle detection, and providing clearer guidance for voice commands. Participants also suggested new functions such as a reverse driving mode, a 180$^\circ$ turn command, the integration of a robotic arm, and inter-robot communication. Furthermore, fallback mechanisms for handling voice-signal loss and additional communication modalities were proposed to improve robustness.

Overall, the results suggest that the proposed system can lower the barrier to human–robot interaction and shows strong potential for service robotics applications in domestic and industrial environments.

\subsubsection{Phase II: Global \textcolor{red}{(Layperson?)} Perception Study \textcolor{red}{(Video-based?)}}
The second group ($N=10$) comprised participants/\textcolor{red}{laypersons} with no prior background in robotics, representing a broad demographic spectrum (ages 8–74 with mean $\bar{x}_{age} = 43.9$, five male and five female participants) across Africa, Asia and Europe. In this second study, participants were asked to give their opinion based on a video of the verbal interaction, as well as the navigation and object tracking of the robot. The qualitative assessment was structured around three key dimensions similar to phase I, but compressed: 
