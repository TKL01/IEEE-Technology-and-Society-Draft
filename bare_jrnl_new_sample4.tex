...
Mobile robots need to reason about contextualized and structured information to plan and achieve goals. Scene graphs have recently been adapted for mobile robots and different layers of abstraction of the real scene have been introduced to this end. Included are the spatial (locations within the scene), semantic (meaning of entities, including their state, affordances, predicates, and further attributes), topological (connectivity between locations), and task-related (robot goals) layers \cite{rana2023sayplan,im2024egtr}. Nodes populated these layers, which differ in various ways. The update rates of node attributes are one example of such a differentiation \cite{rana2023sayplan,im2024egtr}. For instance, while the initial and final positions of the robot may be known in advance and thus fixed, the achievement of the task itself  may be required within a short time window. To this end, the robot is commanded to complete fast motions. By contrast, the desired position of the robot varies with time in object-following applications.
  \begin{table}[!t]
	\centering
...
Scene graphs have been deployed for different societal applications \cite{amiri2022reasoning}. Finding someone a place to sit in an indoor environment \cite{ravichandran2025safety}, assistance in cooking \cite{kwon2025large}, and  inspection missions in city-scale outdoor environments \cite{viswanathan2024xflie} are application examples. Although most scene graphs capture information from an image, human activities are usually characterized by a long-term horizon beyond a single image.
