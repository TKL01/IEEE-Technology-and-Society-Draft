...
\begin{figure}[!htpb]
	...
	\caption{Timing breakdown of Gemma 3 and Qwen 2.5 VL models based on size. The suffix "Husky" indicates that the model has been modified using modelfiles and adapted to our application. Abbreviations: "G3" = Gemma 3, "it qat" = instruction-tuned and Quantization-Aware Training, "Q4" = 4-Bit quantization.}
...
The questions were rated by participants (P1 to P10) on a scale of 1 to 5, with 1 being the lowest/worst rating and 5 being the highest/best rating. For questions Q3 and Q6, negative items were reverse-coded for statistical analysis. For every question (Q1 to Q9) a mean $\bar{x}$ and a sample standard deviation $s$ was computed to make a quantitative statement. Since participant P5 skipped question Q3, n = 9 was used to calculate mean and standard derivation for this particular question.

\subsubsection{Results and Discussion}

\begin{table}[htpb]
	\centering
	\caption{Quantitative Results of the Usability Evaluation ($N=10$) featuring Mean Score $\bar{x}$ and Sample Standard Deviation $s$. Scoring: 5 = highest/best, 1 = lowest/worst.
	*Negative questions: lower values indicate better performance.}
	...
	\begin{tabular}{|l|c|c|c|c|c|c|c|c|c|}
		...
		P9 & 5 & 4 & 2 & 5 & 4 & 2 & 5 & 4 & 5 \\ \hline
		P10 & $-$ & $-$ & $-$ & $-$ & $-$ & $-$ & $-$ & $-$ & $-$ \\ \hline \hline
		\textbf{Mean} $\bar{x}$ & \textbf{} & \textbf{} & \textbf{} & \textbf{} & \textbf{} & \textbf{} & \textbf{} & \textbf{} & \textbf{} \\ \hline
		\textbf{SD} $s$ & \textbf{} & \textbf{} & \textbf{} & \textbf{} & \textbf{} & \textbf{} & \textbf{} & \textbf{} & \textbf{} \\ \hline
	\end{tabular}
	\vspace{1ex}
\end{table}


The quantitative results in Table \ref{tab:user_study_results} indicate a high level of user satisfaction across all categories. Particular emphasis was placed on the ease of use of voice commands, the subsequent correct object identification by the system, the system response time, and the perceived safety during robot operation (Q1, Q4, Q5 and Q8: $\bar{x}$ = X/5 and in each case $s$ = X, . Furthermore, the reliability of object recognition was rated as very good ($\bar{x}_{Q2}$ = X and $\bar{x}_{Q3}$ = X) and the overall operation of robot and GUI ($\bar{x}_{Q7}$ = X) was also rated very highly. The participants are also willing to use such a system in real-world applications ($\bar{x}_{Q9}$ = X). 
While $\bar{x}_{Q6}$ = X further confirms that the tracking behavior was perceived as predictable and non-threatening. 
The low standard deviation $s$ across most items indicates a high consensus among participants.
It should be noted, however, that the participants all come from the field of engineering, so that a high level of technical affinity and an intrinsic interest in robotics must be assumed. Therefore, the acceptance to use the system in real-world applications as well as the perceived ease of use might be lower for lay users. The study is also limited by the small sample size ($N=10$), which serves as a pilot evaluation.  
...
We would like to thank Mr. S. Azizpour and Mr. A. Koennecke for his support in conducting the usability study and robot control, respectively. We would also like to thank all participants for their time and feedback.
